...
class SingBoxRunner:
    ...
    async def start(self, outbounds: list[dict]) -> ClashApiConfig:
        ...
        config_outbounds = [{"type": "direct", "tag": "DIRECT"}, *outbounds]

        config = {
            "log": {"level": "warn"},
            "inbounds": [
                {"type": "mixed", "tag": "mixed-in", "listen": "127.0.0.1", "listen_port": 10809}
            ],
    ...
    async def _wait_ready(self, secret: str) -> None:
        ...
        async with httpx.AsyncClient(timeout=2) as client:
            for _ in range(40):
                ...
                try:
                    r = await client.get(f"http://{self._host}:{self._port}/proxies", headers=headers)
    ...
    async def delay_test(self, api: ClashApiConfig, proxy_name: str, url: str, timeout_ms: int) -> int | None:
        ...
        async with httpx.AsyncClient(timeout=(timeout_ms / 1000.0) + 2) as client:
            r = await client.get(f"{api.base_url}/proxies/{proxy_name}/delay", headers=headers, params=params)
            if r.status_code != 200:
                return None
            data = r.json()
